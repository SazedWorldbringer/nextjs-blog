...
export function getSortedJournalEntriesData() {
  // Get file names under /posts
  const fileNames = fs.readdirSync(journalDirectory);
  const allJournalEntriesData = fileNames.map((fileName) => {
  ...
  // Sort posts by date
  return allJournalEntriesData.sort((a, b) => {
    if (a.date < b.date) {
      return 1;
    } else {
      return -1;
...
export function getAllJournalEntriesId() {
  const fileNames = fs.readdirSync(journalDirectory);
...
export async function getJournalEntryData(id: string) {
